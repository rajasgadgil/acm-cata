...
<?php if ( $settings['show_date'] ) { ?>
	<li>
		<i class="fa fa-calendar"></i><a href="<?php the_permalink(); ?>"><time itemprop="datePublished" class="entry-date published" datetime="<?php the_time('c') ?>"><?php echo get_the_date(); ?></time></a>
		<time itemprop="dateModified" class="entry-date updated hidden" datetime="<?php the_modified_time('c'); ?>" ><?php the_modified_date(); ?></time>
	</li>
<?php } ?>
<?php if ( $authorId ) { ?>
	<li><i class="fa fa-user"></i><a href="<?php echo esc_url( get_author_posts_url( $authorId ) ) ?>"><span itemprop="author" itemscope itemtype="http://schema.org/Person" class="vcard author"><span class="fn" itemprop="name"><?php echo get_the_author(); ?></span></span></a></li>
<?php } ?>
