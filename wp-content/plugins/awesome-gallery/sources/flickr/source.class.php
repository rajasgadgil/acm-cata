...
class ASG_Flickr_Source extends ASG_Http_Source {
	...
	function get_photo_url($data, $allow_smaller = false, $options = array()) {
		if (isset($this->source['image_size']) && $this->source['image_size']) {
			...
			return "//farm{$data['farm']}.staticflickr.com/{$data['server']}/{$data['id']}_{$data['secret']}_{$letter}.jpg";
		}
	...
	function get_photosets() {
		...
		foreach ($data['photosets']['photoset'] as $photoset) {
			$result [] = array(
				'id' => $photoset['id'],
				'cover' => $this->get_photo_url($photoset),
	...
	function get_groups() {
		...
		foreach ($data['groups']['group'] as $group) {
			$result [] = array(
				'id' => $group['nsid'],
				'cover' => ($group['iconserver'] > 0 ? '//farm' . $group['iconfarm'] . '.staticflickr.com/' . $group['iconserver'] . '/buddyicons/' . $group['nsid'] . '.jpg' : 'www.flickr.com/images/buddyicon.gif'),
				'title' => $group['name']
			);
		}
		return $result;
	...
	function fetch_raw_images2($page, $per_page, $options) {
		...
		switch ($this->source['source_type']) {
			...
			case 'photoset':
				...
				$data = $this->flickr_get('flickr.photosets.getPhotos', $params,
					$options);
